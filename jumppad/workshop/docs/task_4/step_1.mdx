...
Note: The GCP Minecraft server is already running and accessible via SSH on port 22. The location you retrieved
in the previous task from Blackbeard's treassure chest.
...
* Hint: you will need to set the `organization` property to your organization id, use the locals provided
...
* Hint: you will need to set the variables `ansible_host` and `ansible_port` to the value from Blackbeard's treasure chest and the ssh port 22 respectively.
...
<Solution title="Solution: AAP for shared GCP Minecraft configuration">
