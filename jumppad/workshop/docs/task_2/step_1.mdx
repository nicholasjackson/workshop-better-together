...
More information can be found in the <a href="https://developer.hashicorp.com/hcp/docs/vault/get-started/manage-public-access" target="_blank" rel="noopener noreferrer">manage public network access</a> documentation.
...
Last WAL                 800808
```
5. Login using the `userpass` method with the team's credentials by using the command, replace `<your_team_name>` and `<password>` with your team's credentials.
```
