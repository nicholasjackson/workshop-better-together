...
3. From the left menu, click on **Automation Execution** and then **Credentials**.
**`hashicorp_vault_ssh_approle` has been preconfigured but you'll need to input the AppRole Role ID and Secret ID to complete the configuration**
...
6. Click **Test** (Opens a new window with additional fields).
    a. For `Unsigned Public Key`, insert a unsigned public key. You can use the key provided in the `secrets/app` path in Vault or use your own if you are feeling adventurous.
    b. For `Path to Secret`, insert `ssh`.
    c. For `Path to Auth`, insert `approle`.
    d. For `Role Name`, insert `aap`.
