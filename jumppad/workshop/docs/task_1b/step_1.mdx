...
2. The <Link href="https://developer.hashicorp.com/vault/docs/commands" target="_blank" rel="noopener noreferrer">Vault CLI</Link> is pre-installed in the terminal. The `VAULT_ADDR` environment variable has also been set for you.

You also sete the `TEAM_NAME` environment variable to your team name in the previous step so you are ready
to go.
