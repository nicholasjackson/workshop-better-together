...
```
_Replace `<team_name>` and `<password>` with your team's credentials_.
...
```
I hold the whispers of the wind and the stories of the sea. 
To enter my chamber, seek the city of sin and the state of silver.
```
5. Find the Vault secret key using:
```bash
vault kv get -namespace=admin/<team_name> secrets/skofy26
```
_Replace `<team_name>` with your team's name_.
